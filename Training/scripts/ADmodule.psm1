...
filter Get-GGUIDMMap {
 ...
 Param (
    ...
    $SearchString = "LDAP://CN=Schema,CN=Configuration,DC=xxx,DC=xxxx,DC=xxx,DC=xxx"
    ...
    #>
    $SearchString = "LDAP://CN=Extended-Rights,CN=Configuration,DC=xxx,DC=xxx,DC=xxx,DC=xxx"
    $Searcher = New-Object System.DirectoryServices.DirectorySearcher([ADSI]$SearchString)
    $Searcher.PageSize = $PageSize
    $Searcher.CacheResults = $False
...
function Get-AADOObjectIInfo {
    ...
    Write-Host -Fore Green `n$object
    ...
    ########### AD Groups  ####################
		...
		$elev_groups = @(
		'xxx\Account Operators'
		'xxx\Administrators'
		'xxx\Allowed RODC Password Replication Group'
		'xxx\Backup Operators'
		'xxx\Certificate Service DCOM Access'
		'xxx\Cert Publishers'
		'xxx\Distributed COM Users'
		'xxx\DnsAdmins'
		'xxx\Domain Admins'
		'xxx\Enterprise Admins'
		'xxx\Event Log Readers'
		'xxx\Group Policy Creators Owners'
		'xxx\Hyper-V Administrators'
		'xxx\Pre–Windows 2000 Compatible Access'
		'xxx\Print Operators'
		'xxx\Protected Users'
		'xxx\Remote Desktop Users'
		'xxx\Schema Admins'
		'xxx\Server Operators'
		'xxx\WinRMRemoteWMIUsers_'
 	)
    ...
    if ($o) {
        $o | %{Get-GPO -guid $_ | Select Id,DisplayName,Owner}
    }
    else {
        Write-Host "None"`n
    }
...
$funcs =  @('Get-NNetUUser'
			'Get-NNetCComputer'
			'Get-NNetGGroup'
			'Get-NNetGGroupMMember'
			'Get-NNetDDomain'
			'Get-NNetFForest'
			'Get-NNetGGpo'
            'Get-NNetGGpoGGroup'
            'Get-OObjectAAcl'
            'Convert-LLDAPPProperty'
            'Convert-NNameTToSSid'
            'Convert-SSidTToNName'
            'Convert-AADNName'
            'Get-AADOObject'
            'Get-DDomainSSID'
            'Get-GGptTTmpl'
            'Get-GGUIDMMap'
            'Get-GGroupsXXML'
            'Get-DDomainSSearcher'
            'Invoke-AACLSScanner'
            'Get-AADOObjectIInfo'
            'Get-UserRightsAssignments'
            'Get-LinkedGpos'
            'Get-SpnEncryptedTicket')
